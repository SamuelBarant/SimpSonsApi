    ...
    fun setupObserver(){
        viewModel.uiState.observe(viewLifecycleOwner,{ uiState ->
            if (uiState.isLoading){
                ...
                binding.nameCharacter.text = getString(R.string.error).plus(" ").plus(uiState.error.message)
            } else if (uiState.data != null){
                binding.nameCharacter.text = uiState.data.name
                binding.ageCharacter.text = uiState.data.age.toString().plus(" ").plus(getString(R.string.ageSuffix))
